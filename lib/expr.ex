defmodule AshSql.Expr do
  ...
  defp default_dynamic_expr(
    ...
    case {dynamic_type, row_token} do
      ...
      {dynamic_type, nil} ->
        {Ecto.Query.dynamic(fragment("ash_raise_error(?::jsonb, ?)", ^encoded, ^dynamic_type)), acc}

      {nil, row_token} ->
        {Ecto.Query.dynamic(fragment("ash_raise_error_immutable(?::jsonb, ?)", ^encoded, ^row_token)), acc}

      {dynamic_type, row_token} ->
        {Ecto.Query.dynamic(fragment("ash_raise_error_immutable(?::jsonb, ?, ?)", ^encoded, ^dynamic_type, ^row_token)), acc}
    end
  end
